...
int main(int argc, char **argv) {
  ...
  // Создаем pipe если используем pipe-based коммуникацию
  if (!with_files) {
    if (pipe(pipefd) == -1) {
      perror("pipe failed");
      free(array);
    ...
    if (child_pid >= 0) {
      ...
      if (child_pid == 0) {
        ...
        if (with_files) {
          ...
          close(pipefd[0]); // закрываем чтение в child
  ...
  // Закрываем pipe если использовали
  if (!with_files) {
    close(pipefd[0]);
    close(pipefd[1]);
  }
